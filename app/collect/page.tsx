...
export default function CollectPage() {
    ...
    useEffect(() => {
        const fetchUserAndTask = async () => {
            ...
            try {
                ...
                const fetchTasks = await getWasteCollectionTasks();
                //@ts-ignore
                setTasks(fetchTasks);
            } catch (error) {
                console.error("Error fetching user and tasks", error);
                toast.error("Failed to load user and tasks. Please try again later.");
            } finally {
                setLoading(false);
            }
        }
    ...
    const handleStatusChange = async (tasksId: number, newStatus: CollectTask['status']) => {
        if (!user) {
            toast.error("Please Login to collect Waste.");
            return;
        }
        try {
            const updatedTask = await updateTaskStatus(tasksId, newStatus, user.id);
            if (updatedTask) {
                setTasks(
                    tasks.map((task) => task.id === tasksId ? { ...task, status: newStatus, collectorId: user.id } : task)
                );
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            const imageParts = [
                {
                    inlineData: {
                        data: base64Data,
                        mimeType: 'image/jpg', // Adjust this if you know the exact type
                    },
                },
            ];
            ...
            const text = await response.text();  // Ensure it's a string

            // Clean the response text to remove unwanted markdown and newlines
            const cleanText = text.replace(/```json\n|\n```/g, '').trim();

            try {
                // Attempt to parse the cleaned JSON text
                const parsedResult = JSON.parse(cleanText);  // Now try to parse the cleaned text
                ...
                toast.error('Failed to process verification data.');
            }
        } catch (error) {
            console.error('Error verifying waste:', error);
            setVerificationStatus('failure');
            toast.error('Error during waste verification.');
        }
    };
...
function StatusBadge({ status }: { status: CollectTask['status'] }) {
    const statusConfig = {
